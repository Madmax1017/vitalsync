...
import { FiUsers, FiSearch, FiLoader, FiFilter, FiActivity, FiUser, FiMapPin, FiX, FiPlus, FiFileText, FiCheckSquare } from 'react-icons/fi';
...
export default function NursePatientsPage() {
    ...
    useEffect(() => {
        fetchData();
        const sub1 = supabase.channel('nurse-patients-db')
    ...
    const fetchData = async () => {
        const [pRes, mRes, tRes] = await Promise.all([
            // Filter only to patients assigned to this nurse if required.
    ...
    return (
        <div className="flex min-h-screen w-full bg-[#f8fafc] overflow-hidden">
            <div className="fixed inset-0 pointer-events-none z-0">
                <div className="absolute top-[10%] right-[10%] w-[500px] h-[500px] bg-rose-400/5 rounded-full blur-[140px]" />
                <div className="absolute bottom-[20%] left-[5%] w-[400px] h-[400px] bg-sky-400/5 rounded-full blur-[140px]" />
            </div>
            ...
            <div className="flex-1 flex flex-col h-screen relative z-10 w-full min-w-0">
                <div className="p-4 md:p-6 lg:p-8 flex-1 overflow-y-auto space-y-8">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                        <div>
                            <h1 className="text-3xl font-extrabold tracking-tight text-[#1e1b32]">Assigned Patients</h1>
                            <p className="text-[#64748b] font-medium mt-1">Review active assignments and required clinical care</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 mb-6">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-white/60 bg-white/60 focus:bg-white focus:ring-2 focus:ring-rose-500/20 outline-none transition-all text-sm font-medium"
                            />
                        </div>
                        <div className="relative">
                            ...
                            <select
                                ...
                                className="pl-10 pr-8 py-2.5 rounded-xl border border-white/60 bg-white/60 focus:bg-white focus:ring-2 focus:ring-rose-500/20 outline-none transition-all text-sm font-medium appearance-none min-w-[150px]"
                            >
                    ...
                    {loading ? (
                        <div className="py-20 flex flex-col items-center justify-center">
                            <FiLoader className="w-10 h-10 animate-spin text-rose-400 mb-4" />
                            <span className="text-sm font-bold text-[#64748b] uppercase tracking-widest">Loading Patient Care List...</span>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                            ...
                            <div>
                                <h3 className="text-xl font-bold text-[#1e1b32]">No patients</h3>
                            </div>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                            {filteredPatients.map(p => (
                                <div key={p.id} className={`glass border rounded-2xl p-5 transition-all duration-300 relative group cursor-pointer 
                                    ${p.status === 'Critical' ? 'border-rose-200/50 hover:shadow-rose-500/10' : 'border-white/40 hover:shadow-rose-500/10'} 
                                    hover:shadow-xl hover:-translate-y-1.5`} onClick={() => setSelectedPatient(p)}>
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-rose-100 to-pink-100 flex items-center justify-center text-rose-600 font-bold text-lg">
                                                {p.name?.charAt(0)}
                                            </div>
                                            <div>
                                                <h3 className="font-bold text-[#1e1b32] leading-tight group-hover:text-rose-600 transition-colors">{p.name}</h3>
                                    ...
                                    <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                                        ...
                                        <span className="text-[11px] font-bold text-rose-600 opacity-0 group-hover:opacity-100 transition-opacity">View Details &rarr;</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto relative z-10 shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-6 md:p-8 flex-1">
                            ...
                            <div className="flex items-center gap-4 mb-2">
                                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-rose-500 to-pink-600 flex items-center justify-center text-white font-bold text-2xl shadow-lg shrink-0">
                            ...
                            <div className="flex flex-wrap gap-3 mb-8 mt-6">
                                <button onClick={() => navigate('/nurse/medications')} className="flex items-center gap-2 px-4 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold rounded-xl transition-colors text-sm">
                                    <FiPlus className="w-4 h-4" /> Administer Meds
                                </button>
                                <button onClick={() => navigate('/nurse/tasks')} className="flex items-center gap-2 px-4 py-2 bg-sky-50 hover:bg-sky-100 text-sky-700 font-bold rounded-xl transition-colors text-sm">
                                    <FiCheckSquare className="w-4 h-4" /> View Open Tasks
                                </button>
                            </div>
                            ...
                            <div className="space-y-8">
                                ...
                                <div>
                                    <h4 className="text-sm font-bold text-[#1e1b32] uppercase tracking-widest mb-4 flex items-center gap-2">
                                        <span className="w-2 h-2 rounded-full bg-emerald-500"></span> Nurse's Med Queue
                                    </h4>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        {medications.filter(m => m.patient_name === selectedPatient.name).length === 0 ? (
                                            ...
                                            <div key={m.id} className="p-3 rounded-xl border border-slate-100 flex flex-col gap-1 hover:border-emerald-200 transition-colors">
                                                <div className="flex justify-between">
                                                    <p className="font-bold text-sm text-[#1e1b32]">{m.medication_name}</p>
                                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${m.status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>{m.status}</span>
                                                </div>
                                ...
                                <div>
                                    <h4 className="text-sm font-bold text-[#1e1b32] uppercase tracking-widest mb-4 flex items-center gap-2">
                                        <span className="w-2 h-2 rounded-full bg-orange-500"></span> Outstanding Tasks
                                    </h4>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        {tasks.filter(t => t.title.includes(selectedPatient.name)).length === 0 ? (
                                            ...
                                            <div key={t.id} className="p-3 rounded-xl bg-slate-50 border border-slate-100 text-sm">
                                                ...
                                                <p className="text-xs text-slate-500">Priority Level: Active</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
